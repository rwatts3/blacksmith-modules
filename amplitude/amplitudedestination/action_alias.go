...
func (a Alias) Load(tk *destination.Toolkit, queue *store.Queue, then chan<- destination.Then) {
	...
	for _, event := range queue.Events {
		for _, job := range event.Jobs {
			...
			// to user aliasing. We add the Amplitude API key just before loading
			// the data so it is not saved in the store adapter.
			b, _ := json.Marshal(d.Mapping)
			data := url.Values{
			...
			res, err := a.client.PostForm("https://api.amplitude.com/usermap", data)
			if err != nil {
			...
			// a "Track" event for the "Alias" event so it appears on the Amplitude
			// history.
			then <- destination.Then{
				Jobs:  []string{job.ID},
				Error: nil,
				OnSucceeded: []destination.Action{
					Track{
						Events: []Event{
							{
								Event:              "Alias",
								UserId:             d.Mapping[0].GlobalUserId,
