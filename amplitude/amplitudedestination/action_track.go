...
/*
Track implements the Blacksmith destination.Action interface for the action
"track". It holds the complete job's structure to load into the destination.
*/
type Track struct {
	env    *Options
	client *http.Client
...
the Track receiver. It allows to transform and enrich the data before
...
func (a Track) Marshal(tk *destination.Toolkit) (*destination.Job, error) {
	...
	// Amplitude in case of a 500, 502, or 504 HTTP error. If the event is
	// related to a product, set a default quantity if needed and calculate
	// the revenue if not set.
	insertId := ksuid.New().String()
	for i := range a.Events {
		a.Events[i].InsertId = insertId

		if a.Events[i].ProductId != "" {
			if a.Events[i].Quantity == 0 {
				a.Events[i].Quantity = 1
			}

			if a.Events[i].Revenue == 0 {
				a.Events[i].Revenue = a.Events[i].Quantity * a.Events[i].Price
			}
		}
	}
